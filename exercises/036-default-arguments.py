...
# The print_from_stream function has an issue when using EvenStream as the default argument. This is because default arguments in Python are only evaluated once when the function is defined, leading to the reuse of the same instance of EvenStream across multiple calls. This results in the state of the EvenStream persisting between calls, which is not the intended behavior.

# To fix this issue, we should create a new instance of EvenStream within the function body whenever stream is not provided. 
# Corrected code:
