...
# The next N lines contain the space separated values of the shoe_size desired by the customer and x(i), the price of the shoe.
...
from collections import Counter
X = int(input())
sizes = dict(Counter(list(map(int, input().split()))))
N = int(input())
purchases = []
for i in range(N):
    purchases.append(list(map(int,input().split())))
fsum = 0
for q in range(len(purchases)):
    if int(purchases[q][0]) in sizes:
        if sizes[int(purchases[q][0])] != 0:
            fsum += int(purchases[q][1])
            sizes[int(purchases[q][0])] -= 1 
print(fsum)
